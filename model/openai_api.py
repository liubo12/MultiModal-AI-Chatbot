...
def chat_GPT(query=""):
    ...
    openai.api_key=myconfig.get_model_openai_api_key()    #openai.api_key
    print('api_key')
    print(myconfig.get_model_openai_api_key())
...
def image_create(content,mysize="512x512",timestamp=False):#size="512x512"  timestamp:在terminal模型下，使用timestamp命名生成图片文件保存在generate目录下
    myconfig=MyConfig()
    openai.api_key=myconfig.get_model_openai_api_key()    #openai.api_key
    print('api_key')
    print(myconfig.get_model_openai_api_key())
    try:
        response=openai.Image.create(prompt=content,n=1,size=mysize)
    except Exception as e:
        print('openai图片生成过程中出错')
        return False
    ...
    headers = {}
    try:
        response_img = requests.request("GET", img_url, headers=headers, data=payload)
    except Exception as e:
        print('openai图片返回过程中出错')
        return False
    if not timestamp:
        with open('./generate/res.jpg','wb') as f:
            f.write(response_img.content)
    else:
        with open('./generate/'+timestamp+'.jpg','wb') as f:
            f.write(response_img.content)
    return True
